...
import { db, Courses, Chapters, Sections, Exercises, Users, Feedback, Notes, Student_Progress, Student_Exercise_Progress } from 'astro:db'
...
const runSeed = Effect.gen(function* () {
  ...
  const courses = yield* (Effect.tryPromise(() => seeders.seedCourses()))
...
export const seedDb = () => {
  ...
  pipe(
    runSeed,
    Effect.withLogSpan('Database Seeding')
  )
)}
